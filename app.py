...
# ─────────────────────────────── GOOGLE SHEETS ──────────────────────────────
# Service-account credentials are stored **only** in Streamlit Secrets
...
with tab_view:
    ...
    client_list = sorted(df["Client Name"].unique())
        ...
        if sel_task != "— choose —":
            ...
            with st.form("edit_form"):
                ...
                with col1:
                    ...
                    phase   = st.selectbox("Phase", ["Profile Discovery","Applications","Initial Discovery"], index=["Profile Discovery","Applications","Initial Discovery"].index(record["Phase"]))
                    status  = st.selectbox("Status", ["Not Started","In Progress","Completed","Waiting on Client"], index=["Not Started","In Progress","Completed","Waiting on Client"].index(record["Status"]))
                with col2:
                    priority = st.selectbox("Priority", ["High","Medium","Low"], index=["High","Medium","Low"].index(record["Priority"]))
                    start_date   = st.date_input("Start",  value=pd.to_datetime(record["Start Date"], errors="coerce") or date.today())
                    due_date     = st.date_input("Due",    value=pd.to_datetime(record["Due Date"],   errors="coerce") or date.today())
                    follow_date  = st.date_input("Follow-Up", value=pd.to_datetime(record["Follow-Up Date"], errors="coerce") or date.today())
                    payment = st.selectbox("Payment", ["Pending","Paid","Overdue"], index=["Pending","Paid","Overdue"].index(record["Payment Status"] or "Pending"))
